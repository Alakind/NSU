package nsu.ivakin.snakeWeb.models;

import nsu.ivakin.snakeWeb.Wrappers.Player;
import nsu.ivakin.snakeWeb.proto.SnakesProto;
import nsu.ivakin.snakeWeb.proto.SnakesProto.GameState.*;
import nsu.ivakin.snakeWeb.util.GameObjectBuilder;
...
public class MasterGameModel {
    ...
    private Map<Integer, nsu.ivakin.snakeWeb.Wrappers.Snake> snakes = new ConcurrentHashMap<>();
    ...
    private void initSnakes(List<Snake> immutableSnakesList) {
        immutableSnakesList.forEach(snake -> snakes.put(snake.getPlayerId(), new nsu.ivakin.snakeWeb.Wrappers.Snake(snake)));
    }
    ...
    private List<SnakesProto.GameState.Snake> getProtoSnakes(){
        return snakes.values().stream().map(nsu.ivakin.snakeWeb.Wrappers.Snake::toProtoSnake).collect(Collectors.toList());
    }

    public Map<Integer, nsu.ivakin.snakeWeb.Wrappers.Snake> getSnakes() {
        return snakes;
    }
    ...
    public void initPlayerHeadDirections(int id) {
        ...
        headDirections.get(id).add(nsu.ivakin.snakeWeb.Wrappers.Snake.getDefaultDirection());
    }
